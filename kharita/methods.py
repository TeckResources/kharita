...
def load_dataframe(input_data):
	...
	for record in input_data.values:
		vehicule_id, timestamp, lat, lon, speed, angle = record
		pt = GpsPoint(vehicule_id=vehicule_id, timestamp=timestamp, lat=lat, lon=lon, speed=speed,angle=angle)
		data_points.append(pt)
		raw_points.append(pt.get_coordinates())
	points_tree = cKDTree(raw_points)
...
def create_trajectories(input_data, waiting_threshold=5):
	"""
	...
	data_points, raw_points, points_tree = load_dataframe(input_data=input_data)
	detections = defaultdict(list)
	for p in data_points:
		detections[p.vehicule_id].append(p)

	# compute trajectories: split detections by waiting_threshold
	logging.info('Computing trajectories')
	trajectories = []
